...
RUN apt-get install -y git libzip-dev nodejs python3 libgtk2.0-0 libgtk-3-0 libgbm-dev libnotify-dev libnss3 libxss1 libasound2 libxtst6 xauth xvfb
...
RUN npm install
RUN npm run build
RUN mkdir -p tests/backend/tmp/repository
